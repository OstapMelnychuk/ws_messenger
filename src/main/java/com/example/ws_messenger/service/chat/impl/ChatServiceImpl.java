...
public class ChatServiceImpl implements ChatService {
    ...
    public Long createChat(ChatDto chatDto)  {
        if (chatRepository.existsByChatName(chatDto.getChatName())) {
            throw new RuntimeException("Chat with the name %s already exists".formatted(chatDto.getChatName()));
        }
    ...
    public Chat findById(Long chatId) {
        return chatRepository.findById(chatId).orElseThrow(() -> new IllegalArgumentException("There is no chat with id %s".formatted(chatId)));
    }
    ...
    public Chat addNewMembersToAChat(Long chatId, Set<Long> chatUserIds) {
        Chat chat = chatRepository.findById(chatId).orElseThrow(() -> new IllegalArgumentException("Chat with id %s not found".formatted(chatId)));
